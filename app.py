...
def song_filter():
    ...
    st.write("Please choose a decade in order to see results.")
    global spotify_df
    # Isolates only certain columns we want shown in the table
    ...
    danceability = st.slider('Danceability (drag slider to desired minimum amount)', float(spotify_2.Danceability.min()),
                             float(spotify_2.Danceability.max()), step=1.)
    popularity = st.slider('Popularity (drag slider to desired minimum amount)', float(spotify_2.Popularity.min()),
                           float(spotify_2.Popularity.max()), step=1.)
    energy = st.slider('Energy (drag slider to desired minimum amount)', float(spotify_2.Energy.min()),
                       float(spotify_2.Energy.max()), step=1.)
    speechiness = st.slider('Speechiness (drag slider to desired minimum amount)', float(spotify_2.Speechiness.min()),
                            float(spotify_2.Speechiness.max()), step=1.)
    acousticness = st.slider('Acousticness (drag slider to desired maximum amount)', float(spotify_2.Acousticness.min()),
                             float(spotify_2.Acousticness.max()), step=1., value=99.)
    length = st.slider('Length in seconds (drag slider to desired maximum amount)', float(spotify_2.Length.min()),
                       float(spotify_2.Length.max()), step=1., value=1412.)
